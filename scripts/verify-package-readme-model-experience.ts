...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/extensions/maxkb': { kind: 'none', reason: 'The local administration tools are invoked by consumers; this package registers no model request context itself.' },
  'packages/extensions/maxkb-panel': { kind: 'none', reason: 'The browser panel is a local presentation surface and registers no model request context.' },
  'packages/extensions/soho-brand': { kind: 'none', reason: 'The browser branding layer changes presentation only and registers no model request context.' },
  ...
  'packages/client/ui-settings-skills': { kind: 'none', reason: 'Browser-side Settings surface; it only manages local skills and registers no model-facing context.' },
  ...
  'packages/host/skill-library': { kind: 'indirect', reason: 'The local catalog delegates model-visible skill discovery and invocation to the existing filesystem provider and ui-skill package.' },
